...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dist = 0
        while next_place and next_place != beehive:
            if next_place.bees and self.min_range <= dist <= self.max_range:
                return rANTdom_else_none(next_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for b in range(len(bee)):
            self.place.bees[b - change].reduce_armor(amount + self_destruct*self.damage)
            if bee[b].armor - amount <= 0:
                change += 1
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.true_queen:
            ...
            while next_place:
                if next_place.ant:
                    if next_place.ant.double_damage == False:
                        next_place.ant.double_damage = True
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        bee_armor = [bee.armor for bee in self.place.bees]
        change = 0
        for b in range(len(bee_armor)):
            self.place.bees[b - change].reduce_armor(self.damage)
            if bee_armor[b] - self.damage <= 0:
                change += 1
        # END Problem Optional 1
