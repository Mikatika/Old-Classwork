...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        scheme_curry = lambda x: scheme_eval(x, env)
        operand = expr.rest.map(scheme_curry)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        binding = {}
        while formals:
            binding[formals.first] = vals.first
            formals = formals.rest
            vals = vals.rest
        new_frame.bindings = binding
        return new_frame
        # END PROBLEM 10
...
def do_and_form(expressions, env):
    ...
    if not is_true_primitive(first):
        return False
    elif expressions.rest == nil:
        return first
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        if names == nil:
            ...
            names = Pair(bindings.first.first, Pair(names.first, names.rest))
            values = Pair(eval_all(bindings.first.rest, env), Pair(values.first, values.rest))
