...
(define (enumerate s)
  ...
  (define (counter x y) 
      (if (null? y)
          nil
          (cons (cons x (cons (car y) nil)) (counter (+ x 1) (cdr y)))))
  (counter 0 s)
  )
  ; END PROBLEM 15
...
(define (nondecreaselist s)
    ; BEGIN PROBLEM 17
    (cond ((null? s) nil)
    ((let ((x (nondecreaselist (cddr s)))) (> (car s) (car (cdr s))) (cons (cons (car s) nil) (cons cons((cadr s) (car x)) (cdr x)))))
    (else (let ((y (nondecreaselist (cdr s)))) (cons (cons (car s) (car y)) (cdr y)))))
    )
    ; END PROBLEM 17

(nondecreaselist (list 1 2 3 1 2 2 1))
